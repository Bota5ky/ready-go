...
/*    	for(int i=0; i<Graph_length[first]; i++)
    	{
    		...
    		second = Graph_Adjacency[first][i];
    		if(vec_indegree[first]==0) continue;
    		visited[second] = 1;
    		visitcut[second] = first;
    		for(int j=0; j<Graph_length[second]; j++)
    		{
    			if(visited[Graph_Adjacency[second][j]]==1)continue;  
    			third =  Graph_Adjacency[second][j];
    			if(vec_indegree[third]==0) continue;
    			visitcut[third] = first;
    			visited[third] = 1;

    			for(int k=0; k<Graph_length[third]; k++)
    			{
    				if(visited[Graph_Adjacency[third][k]]==1)continue;  
    				end = Graph_Adjacency[third][k];
