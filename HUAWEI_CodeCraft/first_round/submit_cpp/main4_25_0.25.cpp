...
int Feed_Link_2[280000][30];
//记录每个顶点的入度，放弃一些点
int vec_indegree[280000];

int task_stack[280000];
...
int table[4]={1,2,4,8};    
    ...
    void *findcycles(void *threadid)
    {
        ...
        int thread_step = task_stack_top / 15;
        int i = (table[tid]-1)*thread_step;
        int end = tid==3?task_stack_top:(table[tid+1]-1)*thread_step;
        int first;
