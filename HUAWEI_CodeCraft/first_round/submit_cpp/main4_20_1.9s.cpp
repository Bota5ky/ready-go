...
public:
    ...
    void dfs6(int &first)
    {
    	...
    	for(int i=0; i<Graph_length[first]; i++)
    	{
    		...
    		for(int j=0; j<Graph_length[p1]; j++)
    		{
    			...
    			if(visitcut[p2]==-2){ans3[ans3_cnt][0]=first;ans3[ans3_cnt][1]=p1;ans3[ans3_cnt++][2]=p2;}
    			visited[p2] = 1; 
    			...
    			for(int k=0; k<Graph_length[p2]; k++)
    			{
	    			...
	    			if(visitcut[p3]==-2){ans4[ans4_cnt][0]=first;ans4[ans4_cnt][1]=p1;ans4[ans4_cnt][2]=p2;ans4[ans4_cnt++][3]=p3;} 
	    			visited[p3] = 1; 
	    			...
	    			for(int l=0; l<Graph_length[p3]; l++)
    				{
		    			...
		    			if(visitcut[p4]==-2){ans5[ans5_cnt][0]=first;ans5[ans5_cnt][1]=p1;ans5[ans5_cnt][2]=p2;ans5[ans5_cnt][3]=p3;ans5[ans5_cnt++][4]=p4;}
		    			visited[p4] = 1; 
		    			...
		    			for(int m=0; m<Graph_length[p4]; m++)
		    			{
			    			...
			    			if(visitcut[p5]==-2){ans6[ans6_cnt][0]=first;ans6[ans6_cnt][1]=p1;ans6[ans6_cnt][2]=p2;ans6[ans6_cnt][3]=p3;ans6[ans6_cnt][4]=p4;ans6[ans6_cnt++][5]=p5;}
			    			visited[p5] = 1; 
			    			...
			    			for(int n=0; n<Graph_length[p5]; n++)
		    				{
				    			...
				    			if(visitcut[p6]==-2){ans7[ans7_cnt][0]=first;ans7[ans7_cnt][1]=p1;ans7[ans7_cnt][2]=p2;ans7[ans7_cnt][3]=p3;ans7[ans7_cnt][4]=p4;ans7[ans7_cnt][5]=p5;ans7[ans7_cnt++][6]=p6;}
		    				}	
		    				visited[p5] = 0;
		    			}
		    			visited[p4] = 0;	
    ...
    void save(const string &outputFile)
    {
        ...
        for(int i=0;i<ans3_cnt;i++)
        {         
            idx=sprintf(buf,"%d,%d,%d\n",map_id[ans3[i][0]],map_id[ans3[i][1]],map_id[ans3[i][2]]);
            fwrite(buf, idx , sizeof(char), fp );
        }

        for(int i=0;i<ans4_cnt;i++)
        {         
            idx=sprintf(buf,"%d,%d,%d,%d\n",map_id[ans4[i][0]],map_id[ans4[i][1]],map_id[ans4[i][2]],map_id[ans4[i][3]]);
            fwrite(buf, idx , sizeof(char), fp );
        }

        for(int i=0;i<ans5_cnt;i++)
        {         
            idx=sprintf(buf,"%d,%d,%d,%d,%d\n",map_id[ans5[i][0]],map_id[ans5[i][1]],map_id[ans5[i][2]],map_id[ans5[i][3]],map_id[ans5[i][4]]);
            fwrite(buf, idx , sizeof(char), fp );
        }

        for(int i=0;i<ans6_cnt;i++)
        {         
            idx=sprintf(buf,"%d,%d,%d,%d,%d,%d\n",map_id[ans6[i][0]],map_id[ans6[i][1]],map_id[ans6[i][2]],map_id[ans6[i][3]],map_id[ans6[i][4]],map_id[ans6[i][5]]);
            fwrite(buf, idx , sizeof(char), fp );
        }


        for(int i=0;i<ans7_cnt;i++)
        {         
            idx=sprintf(buf,"%d,%d,%d,%d,%d,%d,%d\n",map_id[ans7[i][0]],map_id[ans7[i][1]],map_id[ans7[i][2]],map_id[ans7[i][3]],map_id[ans7[i][4]],map_id[ans7[i][5]],map_id[ans7[i][6]]);
            fwrite(buf, idx , sizeof(char), fp );
        }
