...
//Rigister 读入数据
func rigister(fileRoad string, m map[int][]int, inDegree map[int]int, preNode map[[2]int]bool) {
	...
	for { //读入数据，创建连接
		...
		//注册两个ID
		r(num1, num2, m, inDegree, preNode)
	}
...
//Rigister 注册邻接表
func r(num1, num2 int, m map[int][]int, inDegree map[int]int, preNode map[[2]int]bool) {
